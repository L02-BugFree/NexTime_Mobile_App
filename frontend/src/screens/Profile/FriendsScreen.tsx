...
export const FriendsScreen: React.FC = () => {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const data = await getFriendRequests(); // Lưu ý
      console.log("Raw requests data:", JSON.stringify(data));
      setRequests(data || []);
    } catch (e) {
  ...
  const handleAcceptRequest = async (userId: string) => {
    try {
      ...
      await createRoom({ type: "DIRECT", userA: me._id, userB: userId });
      Alert.alert("Thành công", "Đã chấp nhận kết bạn và tạo phòng chat!");
      if (tab === "requests") loadRequests();
      console.log(
        "Accepted friend request and created room with user:",
        userId,
      );
      // Auto switch to chat tab (assuming it is configured in navigation)
  ...
  const renderFriendItem = ({ item }: { item: User }) => (
    <View style={s.userItem}>
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          handleRemoveFriend(item._id, item.displayName || "Người dùng")
        }
      >
    ...
    return (
      <View style={s.userItem}>
        ...
        {!isFriend && !isSent ? (
          ...
          <View style={[s.actionBtnDisabled, { backgroundColor: '#E2E8F0' }]}>
  ...
  return (
    <SafeAreaView style={s.safeArea}>
      <View style={s.container}>
        ...
        <View style={s.tabs}>
          <TouchableOpacity
            style={[s.tab, tab === "friends" && s.activeTab]}
            onPress={() => setTab("friends")}
          >
            <Text style={[s.tabText, tab === "friends" && s.activeTabText]}>
              Bạn bè ({friends.length})
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.tab, tab === "requests" && s.activeTab]}
            onPress={() => setTab("requests")}
          >
            <Text style={[s.tabText, tab === "requests" && s.activeTabText]}>
              Lời mời ({requests.length})
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[s.tab, tab === "search" && s.activeTab]}
            onPress={() => setTab("search")}
          >
            <Text style={[s.tabText, tab === "search" && s.activeTabText]}>
              Tìm kiếm
            </Text>
          </TouchableOpacity>
        </View>
            ...
            ListEmptyComponent={
              <View style={s.empty}>
                <Ionicons
                  ...
                  color="#CBD5E1"
                />
                <Text style={s.emptyText}>Bạn không có lời mời nào.</Text>
              </View>
            }
          />
        ) : (
